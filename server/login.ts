...
export async function initLogin(mobile: string) {
  ...
  await to(page.waitForSelector('.internation-code-input',{timeout:3000}))
  

  await to(page.type('.internation-code-input','86'))

  

  // 等待手机号输入框出现
  await to(page.waitForSelector('#phone-number', { timeout: 3000 }));

  // 输入手机号
  await to(page.type('#phone-number', mobile));


  // 等待手机号输入框出现
  await to(page.waitForSelector('#user-mobile', { timeout: 3000 }));

  // 输入手机号
  await to(page.type('#user-mobile', mobile));
